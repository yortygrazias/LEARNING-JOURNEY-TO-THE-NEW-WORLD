...
class Model:
    ...
    TABLE_NAMES = ["Users", "Info", "Books", "Preference", "Borrowed", "Directory", "Announcement"]
    MAX_ANNOUNCEMENT_ROWS = 30
    ...
    def create_announcement_table(self):
        self.db_cursor.execute(
            f"""
            CREATE TABLE IF NOT EXISTS {Model.TABLE_NAMES[6]} (
            NUMBER INTEGER PRIMARY KEY,
            TO_ANNOUNCE TEXT NOT NULL, 
            TYPE TEXT NOT NULL,
            TAG TEXT NOT NULL,
    ...
    def store_announcement(self, to_announce, announce_type, tag):
        self.db_cursor.execute(
            f"""
            INSERT INTO {Model.TABLE_NAMES[6]} (TO_ANNOUNCE, TYPE, TAG) VALUES (?,?,?)
            """, (to_announce, announce_type, tag)
        )
        self.db_conn.commit()

        self.db_cursor.execute(
            f"""
            SELECT * FROM {Model.TABLE_NAMES[6]} WHERE TO_ANNOUNCE, TYPE, TAG = (?, ?, ?)
            """, (to_announce, announce_type, tag)
        )
    ...
    def load_announcement(self):
        self.db_cursor.execute(
            f"""
            SELECT COUNT(*) FROM {Model.TABLE_NAMES[6]}
            """
        )
        row_count = self.db_cursor.fetchone()[0]
        initial_row = row_count - Model.MAX_ANNOUNCEMENT_ROWS
        final_row = row_count

        if row_count < Model.MAX_ANNOUNCEMENT_ROWS:
            return self.retrieve(f"{Model.TABLE_NAMES[6]}")
        else:
            self.db_cursor.execute(
                f"""
                SELECT TO_ANNOUNCE, TYPE, TAG FROM {Model.TABLE_NAMES[6]} WHERE NUMBER BETWEEN ? AND ?
...
if __name__ == "__main__":
    dm = Model()
    print(dm.retrieve("Announcement"))
